  ...
  useEffect(() => {
    async function fetchProductos() {
      try {
        ...
        const todasVariantes: Variante[] = activos.flatMap((p: Producto) => p.variantes ?? [])
          ...
          <>
            <section id="novedades" className="py-8 bg-white">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 px-4">
                {currentItems.map(({ id, imagen, nombre, descripcion, seleccionado, variantes: prodVariantes }) => {
                  // Usa variantes del producto, o busca en el estado global si no tiene
                  const variantesDelProducto = (prodVariantes && prodVariantes.length > 0)
                    ? prodVariantes
                    : variantes.filter(v => v.id === id)

                  const primeraVariante = variantesDelProducto.length > 0 ? variantesDelProducto[0] : null
                  const variantPrice = primeraVariante ? primeraVariante.precio : 'N/A'
                    ...
                    >
                      <div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:scale-105 transition duration-300">
                        ...
                        <div className="p-4 text-center">
                          ...
                          <p className="text-black text-base font-bold mt-1">S/ {variantPrice}</p>
                        </div>
                      </div>
                    </Link>
                  )
